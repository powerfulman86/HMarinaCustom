...
class Cron(models.Model):
    ...
    def unlink(self):
        ...
        for lines in self:
            self.env['ir.attachment'].browse(lines.dropbox_flow).unlink()
    ...
    def constrains_dropbox(self):
        for line in self:
            if line.backup_destination == 'sftp':
                if no_pysftp:
                    raise exceptions.Warning(_('Missing required pysftp python package\n'
                                               'https://pypi.python.org/pypi/pysftp'))

            if line.backup_destination == 'dropbox':
                if no_dropbox:
                    raise exceptions.Warning(_('Missing required dropbox python package\n'
                                               'https://pypi.python.org/pypi/dropbox'))

                ia = self.env['ir.attachment'].browse(line.dropbox_flow)
                ia.res_model = 'ir.cron'
                ia.res_id = line.id

                flow = pickle.loads(base64.b64decode(ia.datas))
                result = flow.finish(line.dropbox_authorization_code.strip())
                line.dropbox_access_token = result.access_token
                line.dropbox_user_id = result.user_id

            if line.backup_destination == 'google_drive':
                if no_pydrive:
                    raise exceptions.Warning(_('Missing required PyDrive python package\n'
                                               'https://pypi.python.org/pypi/PyDrive'))
                ...
                gauth.Auth(line.dropbox_authorization_code)
                ia.datas = base64.b64encode(pickle.dumps(gauth))

            if line.backup_destination == 's3':
                if no_boto3:
    ...
    def create_backup(self, check=False):
        ...
        if self.backup_destination == 's3':
            ...
            if os.path.exists('/tmp/backup_tmp'):
                os.remove('/tmp/backup_tmp')

            tmp_file = open('/tmp/backup_tmp', 'wb')
            while True:
                chunk = backup_binary.read(1024)
                if not chunk:
                    break
            ...
            s3.meta.client.upload_file('/tmp/backup_tmp', self.s3_bucket_name, 'Odoo Automatic Backup/' + filename, Config=config)
